/**
 * News Page (Refactored)
 *
 * Displays news-related posts in a vertical list with horizontal cards.
...
export function NewsPostCards({ posts }: { posts: SelectPost[] }) {
  return (
    <PostListVertical
      posts={posts}
      id="news-post-cards"
...
export default function News({
  ...
  return (
    <MainLayout isLoggedIn={isLoggedIn} isAdmin={isAdmin} currentPath="/news">
      {/* Page Header */}
      <section class="py-10 md:py-14 border-b border-slate-200 mb-8">
        ...
        <p class="text-slate-600 text-base md:text-lg leading-relaxed max-w-2xl">
          Stay informed with the latest news and announcements from our
          organization. Discover industry insights, product updates, and
      ...
      <div class="pb-16 flex flex-col lg:flex-row gap-8 items-start">
        ...
        <div class="flex-1 min-w-0">
          <NewsPostCards posts={posts} />
        </div>

        {/* Sidebar - News doesn't have categories, only search and tags */}
            ...
            <form method="get" action="/news">
              <div class="flex items-center gap-2 border border-slate-200 rounded-lg px-3 py-2 focus-within:ring-2 focus-within:ring-cyan-500 focus-within:border-cyan-500 bg-white transition-all duration-300">
                ...
                <input
                  ...
                  value={activeSearch}
                  hx-get="/api/news/posts"
                  hx-trigger="input changed delay:300ms"
                  hx-target="#news-post-cards"
                  hx-swap="outerHTML"
