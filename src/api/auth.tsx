...
authRouter.post(
  ...
  async (c) => {
    try {
      const { email, password, redirect } = await c.req.valid("form");
      const db = drizzle(c.env.DB);

      const user = await getUser(db, email.trim().toLowerCase());
      if (!user) {
        return c.html(
          <LogInPage redirect={redirect} error="Invalid email or password." />,
          400,
        );
      }
      ...
      return c.html(<LogInPage error="An error occurred during login." />, 500);
    }
  },
);
...
authRouter.post(
  ...
  async (c) => {
    ...
    const existingUser = await getUser(db, email.trim().toLowerCase());
    if (existingUser) {
      return c.json({ error: "User with that email already exists." }, 400);
    }

    const passwordHash = await new Scrypt().hash(password.trim());
